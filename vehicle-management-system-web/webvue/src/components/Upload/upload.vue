<template>
  <div class="go-upload">
    ...
    <upload-list @on-delete="onDelete" :files="this.files" v-if="listshow"></upload-list>
     <div
...
<script>
import request from '@/components/Upload/js/request';
import MyProgress from '@/components/Upload/com/progress';
import UploadList from '@/components/Upload/com/list';
import { noop } from '@/components/Upload/js/util';
import UploadDragger from '@/components/Upload/com/dragger';
export default {
  name: 'GoUpload',
  ...
  methods: {
    ...
    upload (file) {
      ...
      if(this.files.length==0){
        this.$message.warning("需要上传一个Excel文件！");
        return;
      }
    ...
    //   this.$set(file, 'url', response.data.path); ariel
     //this.$set(file, 'url', response.msg);
      this.onChange(file, this.files);
